...
- Live page now supports Android/FongMi `lives` direct group/channel config, remote M3U/TXT subscription loading, password-protected group prompts, group/channel/source switching, Android-compatible `LIVE_CHANNEL_REVERSE` and `LIVE_CROSS_GROUP` settings, `LIVE_CHANNEL_GROUP`/`LIVE_CHANNEL` persistence, Android-style EPG URL requests with `{name}`/`{date}` placeholders or `?ch=&date=`, date switching, current-program highlighting, and direct ArkUI `Video` playback. EPG logo/name alias mapping, hidden-channel handling, player kernel mapping, live headers, time-shift, and remote-control numeric channel switching remain pending.
...
4. Expand live parity from the current direct M3U/TXT playback and initial EPG list to Android EPG logo/name alias mapping, hidden-channel handling, live headers, time-shift, player-kernel mapping, and numeric remote switching.

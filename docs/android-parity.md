...
- Push page supports manual direct URL playback through the Play page; remote HTTP push server and QR entry remain pending.
...
- Drive page now persists Android-compatible `StorageDrive` records, supports adding/editing/deleting Alist and WebDAV drives, browses Alist v3 `/api/fs/list` and v2 `/api/public/path`, browses WebDAV with `PROPFIND Depth: 1`, applies `HawkConfig.STORAGE_DRIVE_SORT`, detects common video file extensions, resolves Alist `/d/path` and WebDAV direct file URLs, and opens drive videos through Play/history with `sourceKey="_drive"` and `playFlag="drive"`. WebDAV credentials are stored as Android-compatible `url`/`username`/`password`/`initPath` config and copied into `VodInfo.playerCfg` as a Basic Authorization header snapshot. Local directory, file search, and actual Video request-header injection remain pending.
...
5. Complete Drive parity for Local directory, file search, and player-level WebDAV header injection.
6. Expand Apps parity if a privileged/system distribution path is available: automatic installed-app enumeration, launcher icon resources, and system uninstall handoff.
7. Start player capability mapping against HarmonyOS AVPlayer and external-player limitations.

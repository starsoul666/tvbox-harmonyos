...
- Live page supports config `lives` direct channel groups plus M3U/TXT live subscriptions, password-protected groups, group/channel/source switching, Android-compatible reverse/cross-group channel switching settings, channel-number jump, last channel persistence, Android `epg_data.json` logo/name aliases, EPG program lookup, and ArkUI `Video` playback.
